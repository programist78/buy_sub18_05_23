...
function BusinessAdminPanel() {
  ...
  const [leftFilter1, setLeftFilter1] = useState("All Businesses");
  const [argument1, setArgument1] = useState("createdAt");
  const { data: data1, loading: loading1, error: error1 } = useQuery(GET_BUSINESS_INFO_POSTS, {
    variables: { argument: argument1, page: page1.toString() },
  });

  const { data: data1_signedup, loading: loading1_signedup, error: error1_signedup } = useQuery(GET_ACCEPTED_BUSINESS_INFO_POSTS, {
    variables: { argument: argument1, page: page1.toString() },
  });

  

  const { data: data1_addinfo, loading: loading1_addinfo, error: error1_addinfo } = useQuery(GET_UNACCEPTED_BUSINESS_INFO_POSTS, {
    variables: { argument: argument1, page: page1.toString() },
  });
  ...
  return (
      <div className={styles.part}>
        ...
        <div className={styles.filters}>
          ...
          <div onClick={() => setLeftFilter1("Awaiting Approval")} className={`text ${(leftFilter1 == "Awaiting Approval") ? styles.this : ""}`}>Awaiting Approval</div>
          <div className={styles.sortby}>
      <div className={`text ${styles.button}`} onClick={toggleMenu}>
        <p>Sort by columns</p>
        {isOpen ? <IoIosArrowUp /> : <IoIosArrowDown />}
      ...
      {isOpen && (
        <div className={styles.menu}>
          <div onClick={() => setArgument1("createdAt")} className="text">Date</div>
          <div onClick={() => setArgument1("zipCode")} className="text">Zip Code</div>
          <div onClick={() => setArgument1("brandname")} className="text">Business</div>
          <div onClick={() => setArgument1("websiteLink")} className="text">Website</div>
          <div onClick={() => setArgument1("confirmed")} className="text">Action</div>
        </div>
      )}
    </div>
        </div>
...
function PosterAdminPanel() {
  ...
  const [leftFilter1, setLeftFilter1] = useState("All Posters");
  const [argument1, setArgument1] = useState("createdAt");
  const { data: data1, loading: loading1, error: error1 } = useQuery(GET_POSTER_INFO_POSTS, {
    variables: { argument: argument1, page: page1.toString() },
  });
  ...
  return (
      ...
      {isOpen && (
        <div className={styles.menu}>
          <div onClick={() => setArgument1("createdAt")} className="text">Date</div>
          <div onClick={() => setArgument1("fullname")} className="text">Username</div>
          <div onClick={() => setArgument1("selectedSocial")} className="text">Selected Social</div>
          <div onClick={() => setArgument1("selectedReview")} className="text">Selected Review</div>
          <div onClick={() => setArgument1("confirmed")} className="text">Action</div>
        </div>
      )}
    </div>
...
function BusinessWholeAdminPanel() {
  ...
  const [leftFilter1, setLeftFilter1] = useState("All Businesses");
  const [argument1, setArgument1] = useState("createdAt");
  const { data: data1, loading: loading1, error: error1 } = useQuery(GET_BUSINESS_WHOLE_INFO, {
    variables: { argument: argument1, page: page1.toString() },
  });
  ...
  return (
      ...
      {isOpen && (
        <div className={styles.menu}>
          <div onClick={() => setArgument1("createdAt")} className="text">Date Signed Up</div>
          <div onClick={() => setArgument1("brandname")} className="text">Business name</div>
          <div onClick={() => setArgument1("address")} className="text">Address</div>
          <div onClick={() => setArgument1("brandCompletedPosts")} className="text">Total Posts</div>
          <div onClick={() => setArgument1("paidOut")} className="text">Total Paid Out</div>
        </div>
      )}
    </div>
        </div>
...
function PosterWholeAdminPanel() {
  ...
  const [leftFilter1, setLeftFilter1] = useState("All Posters");
  const [argument1, setArgument1] = useState("createdAt");
  const { data: data1, loading: loading1, error: error1 } = useQuery(GET_POSTER_WHOLE_INFO, {
    variables: { argument: argument1, page: page1.toString() },
  });
  ...
  return (
      ...
      {isOpen && (
        <div className={styles.menu}>
          <div onClick={() => setArgument1("createdAt")} className="text">Date Signed Up</div>
          <div onClick={() => setArgument1("fullname")} className="text">Username</div>
          <div onClick={() => setArgument1("phone")} className="text">Contact Info</div>
          <div onClick={() => setArgument1("balance")} className="text">Balance</div>
          <div onClick={() => setArgument1("paidOut")} className="text">Total Paid Out to Date</div>
        </div>
      )}
    </div>
        </div>
